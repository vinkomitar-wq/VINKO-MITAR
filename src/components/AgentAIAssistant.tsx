...
export default function AgentAIAssistant({
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    if (/\b(delete|remove|erase)\b/i.test(text)) {
      confirmDestructive = window.confirm(
        "This may delete one of your proposals. Continue?",
      );
    }
    ...
    try {
      ...
      const response = await fetch("/api/agent-concierge-chat", {
        method: "POST",
        headers: {
      ...
      if (response.status === 401) {
        throw new Error(
          "Your session isn't authorized for the broker assistant.",
        );
      }
      if (!response.ok) {
      ...
      const replyText: string = data?.reply || "Done.";

      const uiActions: any[] = Array.isArray(data?.uiActions)
        ? data.uiActions
        : [];
      for (const ui of uiActions) {
        window.dispatchEvent(
          new CustomEvent("agent-portal-action", {
            detail: { action: ui.action, payload: ui.payload },
          }),
        );
      }
        ...
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center opacity-60">
            ...
            <h3 className="text-sm font-bold text-slate-600 uppercase tracking-widest">
              How can I assist you, Broker?
            </h3>
            <p className="text-[11px] text-slate-500 max-w-xs mt-2">
              Ask me to draft emails for clients, explain our destinations,
              detail our pricing structure, or outline standard itineraries.
            </p>
          </div>
        )}
