...
interface AgentReferralQrGeneratorProps {
  currentAgent: any;
  profileName: string;
  profileWhatsapp: string;
  profilePhone: string;
  profileLineId: string;
  profileWechatId: string;
  cardGreeting: string;
...
export default function AgentReferralQrGenerator({
  currentAgent,
  profileName,
  profileWhatsapp,
  profilePhone,
  ...
  cardTagline,
  setCardTagline,
  initialCoagentId = "main",
}: AgentReferralQrGeneratorProps & { initialCoagentId?: string }) {
  // Custom QR Badge States
  const [selectedCoagentId, setSelectedCoagentId] =
    useState<string>(initialCoagentId);
  ...
  // Compile Dynamic Pairing URL
  const selectedCoagent = currentAgent?.coagents?.find(
    (c: any) => c.id === selectedCoagentId,
  );

  let basePairingUrl = `${getPublicUrl()}/?agent=${encodeURIComponent(currentAgent?.email || "")}&agentName=${encodeURIComponent(profileName || currentAgent?.name || "")}&agentWhatsApp=${encodeURIComponent(profileWhatsapp || currentAgent?.whatsapp || "")}&agentPhone=${encodeURIComponent(profilePhone || currentAgent?.contactPhone || "")}`;

  if (selectedCoagentId && selectedCoagentId !== "main") {
    basePairingUrl += `&coagentId=${encodeURIComponent(selectedCoagentId)}`;
    if (selectedCoagent) {
      basePairingUrl += `&coagentName=${encodeURIComponent(selectedCoagent.name)}`;
      if (selectedCoagent.phone) {
        basePairingUrl += `&coagentPhone=${encodeURIComponent(selectedCoagent.phone)}`;
      }
    }
  }
  ...
  // Handle URL Copy
  const handleCopyUrl = () => {
    navigator.clipboard.writeText(basePairingUrl);
    setQrCopied(true);
  ...
  const triggerLocalTestPairing = () => {
    ...
    if (selectedCoagentId !== "main") {
      localStorage.setItem(
        "phuket_charter_coagent_referred",
        selectedCoagentId,
      );
      const coagentObj = currentAgent?.coagents?.find(
        (c) => c.id === selectedCoagentId,
      );
      if (coagentObj) {
        localStorage.setItem(
          "charter_active_coagent",
          JSON.stringify(coagentObj),
        );
      }
    } else {
      localStorage.setItem("phuket_charter_coagent_referred", "");
      localStorage.removeItem("charter_active_coagent");
    }
    localStorage.setItem(
      `phuket_charter_referred_at_${currentAgent?.email}`,
      new Date().toISOString(),
    );
    localStorage.setItem("phuket_just_scanned_referral", "true");

    // Dispatch custom event to let the general app know if needed
    window.dispatchEvent(
      new CustomEvent("broker_pairing_triggered", {
        detail: {
          agent: currentAgent?.email,
          coagent: selectedCoagentId === "main" ? null : selectedCoagentId,
        },
      }),
    );

    setTestPairingMsg(
      `✓ Success! Browser session permanently assigned to broker '${profileName || currentAgent?.name || "Premium Broker"}'. Booking Requests or Chat messages instantiated from this device are locked securely to your inbox.`,
    );
  ...
  const downloadRefQr = () => {
    ...
    const suffix =
      selectedCoagentId !== "main"
        ? `_coagent_${selectedCoagentId}`
        : "_broker";
    const modeName = qrDisplayTab === "badge" ? "_custom_qr" : "_invite_card";

    downloadLink.download = `${(profileName || currentAgent?.name || "agent").replace(/\s+/g, "_")}${suffix}${modeName}.svg`;
  ...
  const handleShare = async (
    platform: "wa" | "fb" | "mail" | "os" | "ig" | "tiktok",
  ) => {
    let customText = currentAgent?.customShareMessage;
    if (customText) {
      // Ensure the custom text has the link attached if they didn't include it
      if (!customText.includes("http")) {
        customText = `${customText}\n\nVIP Link: ${basePairingUrl}`;
      }
    }
    const textMsg =
      customText ||
      `Check out Phuket Yacht Charters and book your next premium catamaran direct!\n\nVIP Broker Link: ${basePairingUrl}`;
    ...
    try {
      ...
      if (svgElement) {
        ...
        if (blob) {
          shareFile = new File(
            [blob],
            `Phuket_Yacht_Charters_VIP_${qrDisplayTab}.png`,
            { type: "image/png" },
          );
        }
      }
      ...
      if (
        ...
        await navigator.share({
          title: "Phuket Yacht Charters VIP Request",
          text: textMsg,
          files: [shareFile],
        });
        return;
    ...
    if (platform === "wa") {
      ...
      window.open(
        `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(basePairingUrl)}`,
        "_blank",
      );
    } else if (platform === "mail") {
      window.open(
        `mailto:?subject=${encodeURIComponent("Premium Yacht Charter Booking")}&body=${encodeURIComponent(`I would like to invite you to book a premium yacht charter in Phuket!\n\nUse my direct VIP link to access the platform:\n${basePairingUrl}`)}`,
      );
    }
  };
  ...
  const handlePrintLayout = (targetType: "badge" | "poster") => {
    ...
    if (svgElement) {
      ...
      // Do not use image base64 as some browsers strip out linearly referenced gradients in nested forms, render inline!
      qrSvgHtml = svgData;
    }
    ...
    if (targetType === "poster") {
      printWindow.document.write(`
        <html>
        <head>
          <title>Phuket Yacht Charters - Premium VIP Poster</title>
          <style>
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Phuket Yacht Charters - Premium VIP Poster Card</title>
          <style>
            body {
        ...
        <body>
          <div class="plaque-wrapper">
            ...
            <div class="broker-name">${profileName || currentAgent?.name || "Representative"}</div>
            ...
            <div class="notice-badge">SCAN TO LOCK BROKER WORKSPACE</div>
            <div class="sub-notice">Scan this QR link with your iPhone or Android camera to claim secure representative access & explore direct catamaran quotation options.</div>
            
            <div class="contacts-box">
              WhatsApp Direct: +${profileWhatsapp || "Representative"} <br/>
              ${profileLineId ? "LINE ID: " + profileLineId : ""} ${profileWechatId ? " | WeChat ID: " + profileWechatId : ""} <br/>
              <span style="font-size: 8px; opacity:0.5; text-transform:uppercase; letter-spacing:1px; display:block; margin-top:10px;">Deep Cookie Lock System Client Pairing Poster</span>
            </div>
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start font-sans">
        ...
        <div className="lg:col-span-5 bg-white border border-slate-205 p-5 rounded-xs space-y-4 shadow-2xs">
          ...
          {/* Co-Agent / Partner Target Selector */}
          <div className="space-y-1.5">
            ...
            <select
              ...
              onChange={(e) => setSelectedCoagentId(e.target.value)}
              className="w-full text-xs font-sans py-2.5 px-2.5 border border-slate-205 rounded bg-slate-50 text-slate-800 font-bold focus:ring-1 focus:ring-emerald-800 focus:outline-hidden"
            >
              <option value="main" className="font-bold">
                ⭐ {currentAgent?.name || "Main Agent"} (Your Main QR Code)
              </option>
              {(currentAgent?.coagents || []).map((co: any) => (
                <option key={co.id} value={co.id}>
                  👥 Partner: {co.name} (Sub-broker tracking ID: {co.id})
                </option>
              ))}
            </select>
            <p className="text-[8.5px] text-slate-450 leading-relaxed">
              Generate a tailored referral code for you or any sub-broker
              registered under your master console. All customer messages sent
              via their link flow instantly into your live chat console, labeled
              as referred from that particular partner.
            </p>
          </div>
            ...
            {qrDisplayTab === "badge" ? (
              ...
              >
                <div className="text-center">
                  ...
                  <h6
                    className={`font-serif font-black text-white ${selectedCoagentId && selectedCoagentId !== "main" ? "text-xs" : "text-md"} tracking-wider uppercase mt-1`}
                  >
                    {selectedCoagentId && selectedCoagentId !== "main"
                      ? `${currentAgent?.coagents?.find((c: any) => c.id === selectedCoagentId)?.name || "Partner"} - ${currentAgent?.companyName || profileName || currentAgent?.name || "Agent"}`
                      : (
                  ...
                  <p className="text-[8px] font-mono font-bold tracking-widest text-slate-400 mt-0.5">
                    {selectedCoagentId && selectedCoagentId !== "main"
                      ? "DEEP COOKIE LOCKED PARTNER"
                      : "DEEP COOKIE LOCKED REPRESENTATIVE"}
                  </p>
                </div>
              ...
              <div className="relative p-1 rounded bg-[#0D182E] border border-slate-800 shadow-2xl max-w-[360px] w-full">
                ...
                >
                  ...
                  >
                    {selectedCoagentId && selectedCoagentId !== "main"
                      ? `${currentAgent?.coagents?.find((c: any) => c.id === selectedCoagentId)?.name?.toUpperCase() || "PARTNER"} - ${(currentAgent?.companyName || profileName || currentAgent?.name || "AGENT").toUpperCase()}`
                      : (
                          currentAgent?.companyName ||
                          profileName ||
                          currentAgent?.name ||
                  ...
                  <g transform="translate(165, 460)">
                    ...
                    >
                      ...
                      {profileWechatId ? ` | WeChat: ${profileWechatId}` : ""}
                    </text>
                  </g>
          ...
          <div className="bg-slate-50 border border-slate-202 p-4.5 rounded-xs text-left space-y-4 shadow-3xs">
            ...
            <div className="flex flex-wrap gap-2 pt-1 border-b border-slate-200 pb-4 mb-2">
              ...
              <button
                ...
                className="flex-1 min-w-[60px] flex justify-center items-center gap-1.5 py-1.5 bg-[#1877F2]/10 text-[#1877F2] hover:bg-[#1877F2]/20 font-bold text-[10px] uppercase rounded transition-colors cursor-pointer"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
              ...
              >
                <Printer className="h-4 w-4 text-slate-500 shrink-0" /> PRINT
                PLAQUE CARD
              </button>
              <button
                type="button"
